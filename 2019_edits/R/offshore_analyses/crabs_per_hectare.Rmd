...
overall_mean <- mean(seamap_west$cpue_towspd, na.rm = T)
...
seamap_west %>% 
  ...
  filter(cpue_towspd > 0) %>% 
  ggplot(aes(year, cpue_towspd)) +
  #geom_boxplot(aes(group = year)) +
  geom_boxplot(aes(group = year), outlier.shape = NA) +
  ylim(0, 2) +
  labs(x = NULL, y = "Crabs / Hectare")
```
...
```{r}
region_plot_west <- seamap_west %>% 
  filter(cpue_towspd > 0) %>% 
  ggplot(aes(region, cpue_towspd)) +
...
season_plot_west <- seamap_west %>% 
  ...
  ggplot(aes(region, `% Occurrence`)) +
    geom_pointrange(aes(ymin = occurrence_ymin, 
                        ymax = occurrence_ymax, 
                        shape = season, 
                        color = season)) +
...
region_plot_west + season_plot_west
