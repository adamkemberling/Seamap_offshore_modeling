...
## Tweedie Models {.tabset .tabset-pills}

Tweedie distributions are similar to a negative-binomial in their application for datasets that exhibit zero inflation. Tweedie models are applicable in cases where the response is continuous i.e. catch/area.
...
mod1a <- glm(cpue_towspd ~ year_f + region + season + survey_design +  #Additive covariates
            year_f * region,                                           #Interaction terms
            family = tweedie(),
            data = seamap_west)
...
#Standardize to mean
mean_pred <- mean(mod1a.dat$response)
mod1a.dat <- mod1a.dat %>% mutate(Std_Index = response/mean_pred,
                                CV = SE/response,
...
## Negative Binomial Models {.tabset .tabset-fade .tabset-pills}
